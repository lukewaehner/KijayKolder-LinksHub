...
export const videoApi = {
  ...
  async getActive(): Promise<BackgroundVideo | null> {
    ...
    // data is an array, return first or null
  ...
  async setActive(id: string): Promise<void> {
    console.log("🔄 Setting video as active:", id);
    
    // First deactivate all videos
    const { error: deactivateError } = await supabase
      .from("background_videos")
      .update({ is_active: false })
      .eq("is_active", true); // Only update currently active videos
    ...
    if (activateError) {
      console.error("❌ Error activating video:", activateError);
      throw activateError;
    }

    console.log("✅ Successfully set video as active:", id);
  },
};
