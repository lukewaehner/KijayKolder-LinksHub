...
import { Track, BackgroundVideo } from "@/shared/types";
import { trackApi } from "@/shared/lib/supabase/apis/tracks";
import { videoApi } from "@/shared/lib/supabase/apis/videos";
import { fileApi } from "@/shared/lib/supabase/apis/files";
...
import {
  extractMetadata,
  analyzeMetadata,
} from "@/features/metadata/utils/extractor";
...
export default function AdminPanel() {
  ...
  const handleSetVideoActive = async (videoId: string) => {
    try {
      console.log("🎯 Admin: Setting video as active:", videoId);
      await videoApi.setActive(videoId);
      console.log("✅ Admin: Video set as active successfully");
      ...
      alert(
        `Failed to set video as active: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  };
